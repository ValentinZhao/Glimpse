...
import com.jyutwaa.zhaoziliang.glimpse.Fragment.Bilibili.KichikuFragment;
...
import com.jyutwaa.zhaoziliang.glimpse.Presenter.IBilibiliKichikuPresenter;
...
public class IBilibiliKichikuPresenterImpl extends BasePresenterImpl implements IBilibiliKichikuPresenter {

    Context mContext;
    KichikuFragment mIBilibiliKichikuFragment;
    CacheUtil mCacheUtils;
    public IBilibiliKichikuPresenterImpl(Context mContext, KichikuFragment mIBilibiliKichikuFragment) {
        this.mContext = mContext;
        this.mIBilibiliKichikuFragment = mIBilibiliKichikuFragment;
        this.mCacheUtils = CacheUtil.get(mContext);
    }

    @Override
    public void getKichikuTopList() {
        Subscription subscription = ApiManager.getInstance().getBilibiliApiService().getTopList()
                ...
                .map(new Func1<TopListType, TopListType>() {
                    ...
                    public TopListType call(TopListType topListType) {
                        for(TopListTypeItem item : topListType.getKichiku_list().getAllItems()){
                    ...
                    public void onError(Throwable e) {
                        mIBilibiliKichikuFragment.hideProgressbar();
                        mIBilibiliKichikuFragment.showError(e.getMessage());
                    }

                    @Override
                    public void onNext(TopListType topListType) {
                        mIBilibiliKichikuFragment.hideProgressbar();
                        mIBilibiliKichikuFragment.updateList(topListType);
                    }
                });
